...
export async function getSportsPlaces(
  lat: number,
  lng: number,
  radiusMeters: number
  ...
  // Nearby Search for sports/fitness types
  const nearbyBody = {
    includedTypes: ["gym", "fitness_center", "golf_course", "stadium", "sports_complex"],
    maxResultCount: 20,
  ...
  // Text Search for sports/fitness queries
  const textResults: GooglePlacesRaw[] = [];
  await Promise.all(
    SPORTS_QUERIES.map(async (query: string) => {
      const textBody = {
        textQuery: query,
        maxResultCount: 10,
  ...
  // Apply sports filter
  return FilterService.filterSports(withinRadius);
}
